...
// src/api-router/controller.ts
async function orchestrateScan(config, env) {
  try {
    ...
    const evaluated = valuationResult.evaluated || [];
    ...
    const decisionResult = await decisionActor.makeDecisions(evaluated, 5e3);
...
async function getAgentStatus(env) {
  try {
    ...
    const items_purchased = inventory.filter((item) => item.status === "purchased").length;
    const items_listed = inventory.filter((item) => item.status === "listed").length;
    const total_invested = transactions.filter((tx) => tx.type === "purchase").reduce((sum, tx) => sum + tx.amount, 0);
    const potential_revenue = inventory.reduce((sum, item) => sum + (item.estimated_resale || 0), 0);
    ...
    const stats = {
      listings_scanned: 0,
      // This would need to be tracked separately or stored in an actor
      items_purchased,
      items_listed,
      total_invested,
      potential_revenue,
...
var api_router_default = class extends Service {
  async fetch(request) {
    const app = new Hono2();
    app.use("/*", cors());
    ...
    app.post("/config", async (c) => {
      try {
        const body = await c.req.json();
        if (!body.budget || typeof body.minProfitMargin !== "number" || !body.minProfit || !body.maxPerMarketplace || !body.category) {
          return c.json(
